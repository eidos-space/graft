...
Configure the identity written into new commits and annotated tags with the
Git-compatible keys:
...
The default identity is `Graft <graft@example.invalid>`. Unset either key to
restore its default.
...
This is `true` by default. Checkout-style operations and merge operations that
