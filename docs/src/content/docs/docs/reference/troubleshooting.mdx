...
Make sure the database contains a committed transaction, then stage the
physical path.
...
`graft add` reads a consistent SQLite backup and includes committed WAL frames,
...
Close writers and long-lived read transactions for database paths that the
operation needs to replace. Graft refuses to rename a new snapshot over a live
writer and will not delete WAL or shared-memory sidecars that are still in use.
