...
with a legacy gateway that explicitly returns `405 Method Not Allowed` or `501
Not Implemented`, a client MAY retry the existence probe with `GET /raw/<key>`
...
`upload-bundle` is an optional version 1 capability for Git-style clone
...
The service reads the requested ref, lists the repository's immutable keys,
...
```json
{
  ...
  "reference": {
    ...
    "value_hex": "<lowercase hexadecimal ref bytes>"
  },
  "objects": 3
}
```

Exactly `objects` binary frames follow. Each frame is a 4-byte unsigned path
...
`multipart-object` is an optional version 1 capability for immutable objects
...
Content-Length: 0
x-graft-object-bytes: <total decimal bytes>
```

The response identifies the durable session and the parts already stored:
...
Clients MUST send all missing parts in ascending order. Part numbers start at
one. Every non-final part has exactly the advertised part size; the final part
...
x-graft-part-number: <positive decimal number>
```

Re-uploading a part number replaces that part in the same session, which makes
...
body atomically exposes the complete immutable object. `DELETE
/multipart-abort/<key>` with the same header releases an incomplete session.
