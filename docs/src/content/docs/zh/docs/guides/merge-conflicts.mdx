...
- 支持的 rowid table，以及按声明主键识别的 `WITHOUT ROWID` table 中的 disjoint
  row changes；包括 `STRICT` 和复合主键 schema
...
普通 rowid table 的 identity 是整数 rowid；`WITHOUT ROWID` table 则传包含全部
声明主键字段的 JSON object。
