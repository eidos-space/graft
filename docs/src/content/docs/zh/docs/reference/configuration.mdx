...
可以使用与 Git 兼容的 key 配置新 commit 和 annotated tag 写入的身份：
...
默认身份是 `Graft <graft@example.invalid>`。对任意 key 执行 unset 可恢复默认值。

嵌入式应用可以使用 `graft config ... --json` 读取或修改结构化配置，而不必解析
终端文本。
...
默认值是 `true`。Checkout 类操作，以及 apply、resolve、continue 或 abort merge
candidate 的操作，可能把已跟踪 SQLite snapshot materialize 到对应的 repo-relative
database path，例如 `data.sqlite` 或 `sub-app/main.sqlite`。Stage 与 commit 只记录
repository state，不替换实时 worktree 文件。
...
Graft remote service 使用 Git 风格的 `https://host/namespace/repository`。
`graft+https` 是显式兼容 alias；`graft+http` 只用于本地或可信开发环境。
