...
对普通 rowid table，SQLite rowid 是物理行身份。`WITHOUT ROWID` table 则使用声明
...
- `cell_conflict`：显式启用同一行合并后，同一列得到不同值
- `semantic_key_conflict`：insert/insert、insert/update 或 update/update 的结果具有同一配置语义身份

Semantic key 默认使用 `BINARY`，也可以按 table/column 显式启用 SQLite 内建 ASCII
`NOCASE`：
...
Conflict 输出包含双方物理 identity、展示用 semantic key 和 collation。

## 同一行与 Managed Columns
...
启用后，变更列不重叠则组合，同一列值相同则折叠，同一列值不同则保留结构化 cell
conflict；delete/update 仍是 conflict。

Managed column 只允许 Graft 的有限 resolver：

| Resolver | 含义 |
| --- | --- |
| `ignore_for_conflict` | 仅双方都改 managed value 时保留 Ours；单边变化仍保留。 |
| `max`, `min` | 对支持的 SQLite value 选择有序最大或最小值。 |
| `max_timestamp` | 对 integer 或规范化可排序 text timestamp 选择最大值。 |
| `recompute` | replay 时省略该列，并要求应用自行重算和校验。 |

`recompute` 不运行应用代码；它返回 `recompute_required` validation artifact，并物化其余
字段候选。应用重算后调用 `stageMergeSqliteResult`，Graft 捕获精确数据库，在 private
storage 中执行 integrity/FK check 后才 stage。
...
这是应用 schema policy，应该和应用实际创建的 schema 保持一致。该旧配置会归一化成
managed-column 的 `recompute` resolver。

## Typed SDK 与 Policy Freeze

Node/Electron SDK 提供 `getMergePolicy`、`validateMergePolicy`、`setMergePolicy`。
Version 1 是只含有限 resolver 的 data-only ABI。每个 effective policy 有稳定
`policy_token`，set 需要 `expectedPolicyToken`，merge plan 把 token/version 绑定到
`plan_token`。

Three-way apply 会把 effective policy 冻结到 active merge journal；`getMergeStatus`
报告实际 token/version。完成或 abort 并 replan 前不能修改 policy。所有异步 policy SDK
调用都接受 `AbortSignal`。
