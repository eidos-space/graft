...
`graft` CLI 用于本地开发、脚本、应用集成和手动仓库操作。它直接调用 typed
repository service。
...
`graft sql` 使用普通 SQLite 连接打开 worktree 中的物理数据库文件。这是在本地
使用 Graft 最简单的方式。
...
| `graft upgrade` | 下载并安装当前平台最新的稳定版 CLI。 |
...
| `graft diff --no-index [--json] [--rows] <from.sqlite> <to.sqlite>` | 不依赖 repository，直接比较两个物理 SQLite 文件。 |
| `graft delta create --base <base.sqlite> --target <target.sqlite> --output <file> [--json]` | 创建可移植、可自校验的 SQLite page delta。 |
| `graft delta apply --base <base.sqlite> --delta <file> --output <target.sqlite> [--json]` | 把 delta 应用到精确 base，且不覆盖已有输出。 |
| `graft delta inspect <file> [--json]` | 不应用 delta，只校验并显示其元数据。 |
...
在交互式终端中，`graft log` 使用与 Git 一致的 commit block，并打开配置的
pager（依次使用 `GIT_PAGER`、`PAGER`，默认是 `less`）。管道、重定向和 JSON
输出会直接写出。
...
`diff --no-index` 会把两个物理文件分别捕获为一致的 SQLite snapshot，并包含已提交的
WAL frames。它只接受两个 SQLite database file，不能和 repository-only selector
或 `--db` 一起使用。

`delta` command group 同样不依赖 repository，也不能与 `--db` 组合。Create 与 apply
使用一致的 SQLite image，以精确 base/target SHA-256 绑定 delta，逐 page 处理数据，
并且不会覆盖已有输出。`GRAFTD01` layout 与失败规则见
[SQLite Page Delta 规格](https://github.com/eidos-space/graft/blob/main/docs/specs/graft-sqlite-page-delta-1.0.zh.md)。

`diff --content` 会返回一个 changed UTF-8 text artifact 的
...
如果你用普通 SQLite 工具修改了 materialized 或 exported SQLite 文件，先提交
SQLite transaction，再显式暂存这个物理文件：

```bash
graft add data.sqlite
```

`graft add` 会通过 SQLite online backup 读取一致的 committed state，其中包括
已提交的 WAL frames，不需要手动 checkpoint，也不会修改源数据库。它再按 4 KiB
Graft storage chunk 与 staged/HEAD 基线比较，只写入变化的 chunks。物理 SQLite
数据库可以使用更大的合法 page size，例如 8 KiB；这不会改变 Graft 的 4 KiB 存储
粒度。未提交 transaction 不会进入 Graft snapshot。
...
`--row <table> <identity>` 可以把 resolution 缩小到单个 SQLite row conflict。
普通 rowid table 传整数 rowid；`WITHOUT ROWID` table 传包含声明主键字段的 JSON
object。BLOB 主键字段使用 `{ "$blob": "00ff" }`。
