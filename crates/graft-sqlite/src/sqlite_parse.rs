...
impl<'a> TableScanner<'a> {
    pub fn new(reader: &'a VolumeReader) -> Result<Self, ParseError> {
        let header_page = reader.read_page(PageIdx::FIRST)
    ...
    fn parse_header(page: &[u8]) -> Result<DatabaseHeader, ParseError> {
        ...
        tracing::debug!("Parsed SQLite header: page_size={}, pages={}, encoding={:?}", 
            page_size, num_pages, text_encoding);
    ...
    fn scan_table_page(&self, page_num: u32, rows: &mut Vec<TableLeafCell>) -> Result<(), ParseError> {
        if page_num == 0 || page_num > self.header.num_pages {
            return Err(ParseError::InvalidPageNumber);
        }
        
        let page_idx = PageIdx::try_new(page_num)
            .ok_or(ParseError::InvalidPageNumber)?;
        
        let full_page = self.reader.read_page(page_idx)
            .map_err(|_| ParseError::ReadError)?;
        ...
        tracing::debug!("Scanning page {}: type={}, cells={}, offset={}", 
            page_num, header.page_type, header.num_cells, header_offset);
        ...
        match header.page_type {
            ...
            _ => {
                tracing::warn!("Skipping unknown page type {} on page {}", header.page_type, page_num);
            }
        }
    ...
    fn read_leaf_cells(&self, full_page: &[u8], header_offset: usize, header: &BtreePageHeader, rows: &mut Vec<TableLeafCell>) 
        -> Result<(), ParseError> {
        ...
        for i in 0..header.num_cells {
            ...
            tracing::debug!("Leaf cell {}: ptr_offset={}, cell_offset={}", i, ptr_offset, cell_offset);
            
            if let Some(cell) = Self::parse_table_leaf_cell(&full_page[cell_offset..])? {
                rows.push(cell);
            }
        }
    ...
    fn read_interior_cells(&self, full_page: &[u8], header_offset: usize, header: &BtreePageHeader, rows: &mut Vec<TableLeafCell>) 
        -> Result<(), ParseError> {
        ...
        for i in 0..header.num_cells {
            ...
            tracing::debug!("Interior cell {}: ptr_offset={}, cell_offset={}", i, ptr_offset, cell_offset);
    ...
    fn parse_table_leaf_cell(data: &[u8]) -> Result<Option<TableLeafCell>, ParseError> {
        if data.is_empty() {
            return Ok(None);
        }
...
impl Record {
    ...
    pub fn parse(data: &[u8]) -> Result<Self, ParseError> {
        ...
        if header_end > data.len() {
            tracing::error!("Invalid record: header_end={} > data_len={}", header_end, data.len());
        ...
        tracing::debug!("Record::parse: header_size={}, types={:?}, data_len={}, pos after header={}", 
            header_size, types, data.len(), pos);
...
fn parse_value(data: &[u8], pos: usize, type_code: i64) -> Result<(Value, usize), ParseError> {
    tracing::trace!("parse_value: pos={}, type_code={}, data_len={}", pos, type_code, data.len());
    match type_code {
        0 => Ok((Value::Null, 0)),
        ...
        1..=6 => {
            ...
            if pos + len > data.len() {
                tracing::error!("Integer overflow: pos={}, len={}, data_len={}", pos, len, data.len());
        ...
        7 => {
            ...
            if pos + 8 > data.len() {
                tracing::error!("Float overflow: pos={}, len=8, data_len={}", pos, data.len());
                return Err(ParseError::InvalidValue);
            }
        ...
        n if n >= 12 && n % 2 == 0 => {
            ...
            if pos + len > data.len() {
                tracing::error!("Blob overflow: pos={}, len={}, data_len={}", pos, len, data.len());
                return Err(ParseError::InvalidValue);
            }
        ...
        n if n >= 13 && n % 2 == 1 => {
            ...
            if pos + len > data.len() {
                tracing::error!("Text overflow: pos={}, len={}, data_len={}", pos, len, data.len());
                return Err(ParseError::InvalidValue);
            }
...
pub fn read_all_rows(reader: &VolumeReader, root_page: u32) 
