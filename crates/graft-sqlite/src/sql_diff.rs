...
pub fn generate_diff_report(
    ...
    writeln!(&mut output, "Changed tables: {}", row_diff.table_changes.len()).unwrap();
    writeln!(&mut output).unwrap();
    ...
    for table in &row_diff.table_changes {
        ...
        write!(&mut output, "  {}: {} changes", table.table_name, table.changes.len()).unwrap();
        
        let mut parts = Vec::new();
        if inserts > 0 { parts.push(format!("+{inserts} inserts")); }
        if deletes > 0 { parts.push(format!("-{deletes} deletes")); }
        if updates > 0 { parts.push(format!("~{updates} updates")); }
    ...
    writeln!(&mut output).unwrap();
    writeln!(&mut output, "Use 'PRAGMA graft_diff = \"{from_lsn}\",\"{to_lsn}\",\"rows\"' for detailed row-level diff.").unwrap();
